...
{
    QSqlQuery query;
    if (!query.exec("SELECT COUNT(*) FROM Permissions WHERE Path = '" +
    ...
    return std::is_same_v<decltype(static_cast<ePermissions>(query.value(1).toInt())), ePermissions> ? eStatus::Success
                                                                                                     : eStatus::Failure;
}
